...
- Test against a CI build of your own PR. Every commit you push to a PR
  ...
  before continuing.
- Add a line to the `[Unreleased]` section of `CHANGELOG.md`, under
  Added / Changed / Fixed / Removed, in the same pull request as your change.
  Write it for someone holding the device rather than someone reading the
  code: name what the change does, not how it's built. This is what becomes
  the summary at the top of the release notes, so writing it while the change
  is fresh beats reconstructing it from commit subjects months later.
...
Path A does **not** stamp `CHANGELOG.md` for you. The release notes still pick
up whatever is in `[Unreleased]`, so the notes come out right, but the
changelog keeps calling that section unreleased after it has shipped. Tidy it
by hand afterwards, or use Path B, which does it as part of the run. This is
the tradeoff for Path A being three commands you can run during an incident.

### Path B - run the release workflow
...
2. Click "Build & Release Firmware" in the left sidebar
3. Click "Run workflow", and pick the branch to release from
4. Fill in:
   - **Version**: `1.2.0` (no leading `v`)
   - **Release tier**: `rc` (default), `beta`, `alpha`, or `release` for Stable
   - **Iteration**: leave blank to auto-pick the next free one (`rc1`, then
     `rc2`, ...), or name it explicitly
   - **Post-release bump**: `patch` (default) / `minor` / `major` / `skip`.
     Applies to Stable releases only -- an RC series leaves `version.txt` alone
   - **Rehearse**: tick to build and print the plan without pushing or
     publishing anything
   - **Permit a version lower than the highest released one**: leave unticked.
     It exists so a deliberate rollback is possible, and so a misclick isn't
5. Click "Run workflow"

The workflow does the `version.txt` bump, the changelog stamp, the commit, tag
creation and push, the build, the release with its notes and artifacts, and
(for Stable) the post-release bump -- all in one run. Manual `git tag` from
Path A continues to work in parallel.

It refuses to publish rather than publish something wrong: a tag that already
exists, a version below the highest already-released one, a build whose
embedded version string doesn't match the tag, or a changelog with nothing
written in it will each stop the run.

#### Rehearse first when you're unsure

Ticking **Rehearse** runs the whole thing -- version resolution, the bump, the
build, the version check -- and stops before anything is pushed or published.
...
- **The summary at the top** comes from the `[Unreleased]` section of
  `CHANGELOG.md` in the firmware repo, written by hand in
  Added / Changed / Fixed / Removed form. Whoever lands a change adds their
  line in the same pull request, so the summary is already written when the
  release is cut.
- **The full commit list underneath** is generated by GitHub from everything
  since the previous release.

When the release is cut, the workflow stamps the `[Unreleased]` heading with
the version and date and opens a fresh empty one for the next cycle. A release
with an empty `[Unreleased]` section fails on purpose: a release nobody can
read is not ready to ship.
